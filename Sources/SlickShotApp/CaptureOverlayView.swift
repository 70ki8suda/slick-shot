...
final class CaptureOverlayView: NSView {
    ...
    private static func outerReticlePath(in rect: CGRect) -> NSBezierPath {
        ...
        let bottomNotchWidth = min(max(rect.width * 0.16, 22), 40)
        let bottomNotchDepth = min(max(rect.height * 0.11, 10), 20)
        let bottomNotchCenter = rect.midX + min(max(rect.width * 0.08, 10), 22)
        let bottomRightChamfer = min(max(rect.width * 0.08, 12), 24)
        ...
        path.line(to: CGPoint(x: rect.maxX - bottomRightChamfer, y: rect.minY))
        path.line(to: CGPoint(x: bottomNotchCenter + (bottomNotchWidth / 2), y: rect.minY))
        path.line(to: CGPoint(x: bottomNotchCenter, y: rect.minY + bottomNotchDepth))
        path.line(to: CGPoint(x: bottomNotchCenter - (bottomNotchWidth / 2), y: rect.minY))
